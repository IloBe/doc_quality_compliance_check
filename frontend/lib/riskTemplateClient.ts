...
const store = new Map<string, RiskTemplate>();
...
export async function getRiskTemplate(templateId: string): Promise<ApiResult<RiskTemplate>> {
  const found = Array.from(store.values()).find((tpl) => tpl.template_id === templateId);
  if (!found) {
...
export async function updateRiskTemplate(
  ...
  const existing = Array.from(store.values()).find((tpl) => tpl.template_id === templateId);
  if (!existing) {
    return { ok: false, message: 'Template not found.', degradedToDemo: true };
  }
  ...
  const key = Array.from(store.entries()).find(([, tpl]) => tpl.template_id === templateId)?.[0] || templateId;
  store.set(key, updated);
  return { ok: true, message: 'Template updated.', data: cloneTemplate(updated), degradedToDemo: true };
}

export async function aiSuggestRiskRow(_payload: {
  template_type: TemplateType;
